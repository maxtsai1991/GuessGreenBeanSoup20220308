...
class GuessViewModel : ViewModel(){
         ...
         * 1.建立counter屬性資料,到畫面(Activity)上
         * 2.MutableLiveData(androidx.lifecycle) : 能夠改變資料MutableLiveData,使用MutableLiveData是因為能夠產生物件,泛型符號裡面給型態,這個案例是counter所以給Int,在這裡我們不稱Data稱value
    ...
    init {
        ...
        /**
         * fun guess(num: Int){}此方法為處理比對資料 等於 SecretNumber.kt的validate方法 ,畫面是確認按鈕每按一次就執行一次guess該方法
         * 1.不回傳資料
         */
    fun guess(num: Int){
//        var n = counter.value ?: 0                   // ?: 0 (Elvis運算子)說明 : 1.不希望以後是null 2.如果counter有值就拿去使用,如果沒有值就給0
//        n++
//        counter.value = n                           // 在設定counter身上的value等於n
